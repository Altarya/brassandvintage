...
public class SidedGearHolder extends BlockContainer implements IHasModel{
	...
	public static final String NAME = "sided_gear_holder";
	...
	public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity, boolean nothingProbably){
		for(EnumFacing side : EnumFacing.values()){
			if(worldIn.getTileEntity(pos).hasCapability(RotaryCapabilities.AXLE_HANDLER_CAPABILITY, side)){
	...
	private AxisAlignedBB getAimedSide(TileEntity te, Vec3d start, Vec3d end, boolean useCenter){
		...
		for(int i = 0; i < 6; i++){
			if(te.hasCapability(RotaryCapabilities.AXLE_HANDLER_CAPABILITY, EnumFacing.getFront(i))){
		...
		return SuperMath.rayTraceMulti(list, start, end);
	}
