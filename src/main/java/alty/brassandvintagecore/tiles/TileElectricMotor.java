...
public class TileElectricMotor extends TileMasterAxis{

	public TileElectricMotor(){
		this(EnumFacing.NORTH);
	}

	public TileElectricMotor(EnumFacing facingIn){
		super();
		facing = facingIn;
	...
	public void update(){
		...
		if(neighbor != null && neighbor.hasCapability(CapabilityEnergy.ENERGY, facing)){
			...
			if(handler.canExtract()){
				energyHandler.setEnergy(energyHandler.getEnergyStored() + handler.extractEnergy(efficiency*10, false));
				markDirty();
			}
		}
		
		if(energyHandler.getEnergyStored() >= 1) {
			energyHandler.extractEnergy(10, false);
			gear.addEnergy(.1D * efficiency * 100, true, true);
		}
	}
	...
	private final IAxleHandler gear = new InternalGear();
