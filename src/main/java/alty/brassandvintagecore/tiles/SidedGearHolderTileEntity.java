...
public class SidedGearHolderTileEntity extends TileEntity implements ITickable, IStringReceiver, ISpinReceiver{
	...
	public boolean hasCapability(Capability<?> capability, @Nullable EnumFacing facing){
		if((capability == RotaryCapabilities.AXLE_HANDLER_CAPABILITY || capability == RotaryCapabilities.COG_HANDLER_CAPABILITY) && facing != null && members[facing.getIndex()] != null){
			return true;
		}
	...
	public <T> T getCapability(Capability<T> capability, @Nullable EnumFacing facing){
		if(capability == RotaryCapabilities.COG_HANDLER_CAPABILITY){
		...
		if(capability == RotaryCapabilities.AXLE_HANDLER_CAPABILITY){
			if(facing == null || members[facing.getIndex()] == null){
				return null;
			}
	...
	private class SidedAxleHandler implements IAxleHandler{
		...
		public void propogate(IAxisHandler masterIn, byte key, double rotRatioIn, double lastRadius){
			...
			if(sideTE != null){
				if(sideTE.hasCapability(RotaryCapabilities.AXIS_HANDLER_CAPABILITY, EnumFacing.getFront(side).getOpposite())){
					sideTE.getCapability(RotaryCapabilities.AXIS_HANDLER_CAPABILITY, EnumFacing.getFront(side).getOpposite()).trigger(masterIn, key);
				}
				if(sideTE.hasCapability(RotaryCapabilities.SLAVE_AXIS_HANDLER_CAPABILITY, EnumFacing.getFront(side).getOpposite())){
					masterIn.addAxisToList(sideTE.getCapability(RotaryCapabilities.SLAVE_AXIS_HANDLER_CAPABILITY, EnumFacing.getFront(side).getOpposite()), EnumFacing.getFront(side).getOpposite());
				}
				if(sideTE.hasCapability(RotaryCapabilities.AXLE_HANDLER_CAPABILITY, EnumFacing.getFront(side).getOpposite())){
					sideTE.getCapability(RotaryCapabilities.AXLE_HANDLER_CAPABILITY, EnumFacing.getFront(side).getOpposite()).propogate(masterIn, key, EnumFacing.getFront(side).getAxisDirection() == AxisDirection.POSITIVE ? -rotRatio : rotRatio, 0);
				}
			}
			...
			for(int i = 0; i < 6; ++i){
				if(i != side && i != EnumFacing.getFront(side).getOpposite().getIndex()){
					...
					if(adjTE != null && adjTE.hasCapability(RotaryCapabilities.COG_HANDLER_CAPABILITY, EnumFacing.getFront(side))){
						adjTE.getCapability(RotaryCapabilities.COG_HANDLER_CAPABILITY, EnumFacing.getFront(side)).connect(masterIn, key, rotRatio, .5D);
					}

					// Diagonal gears
					TileEntity diagTE = world.getTileEntity(pos.offset(facing).offset(EnumFacing.getFront(side)));
					if(diagTE != null && diagTE.hasCapability(RotaryCapabilities.COG_HANDLER_CAPABILITY, facing.getOpposite()) && DefaultAxleHandler.canConnectThrough(world, pos.offset(facing), facing.getOpposite(), EnumFacing.getFront(side))){
						diagTE.getCapability(RotaryCapabilities.COG_HANDLER_CAPABILITY, facing.getOpposite()).connect(masterIn, key, rotRatio, .5D);
					}
				}
			}
		}
		...
		private void updateStates(boolean sendPacket){
			...
			if(members[side] == null){
				...
				inertia[side] = 0.125D * SuperMath.betterRound(members[side].getDensity() / 8, 1);
			...
			if(sendPacket && !world.isRemote){
				...
				System.out.println(new TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 512));
				NetworkPacketManager.network.sendToAllAround(msg, new TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 512));
			}
		}
