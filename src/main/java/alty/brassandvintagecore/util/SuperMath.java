...
public class SuperMath {
	...
	public static NBTTagCompound getPlayerTag(EntityPlayer playerIn){
		...
		if(!tag.hasKey(BaVInitialization.MODID)){
			tag.setTag(BaVInitialization.MODID, new NBTTagCompound());
		}
		NBTTagCompound out = tag.getCompoundTag(BaVInitialization.MODID);
